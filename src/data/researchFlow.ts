...
export const informedConsentParagraphs = [
  ...
  '正式测评需要填写姓名、学号、手机号中的至少一项，用于参与登记、后续联系和数据核对。身份信息与测评过程数据将分表保存，并通过匿名 participant_id 关联。',
  '只有授权研究管理员可以访问和导出身份信息。管理员后续导出的研究汇总文件可能包含身份字段。数据将长期保留，并由管理员按照研究管理要求永久删除。',
  '快速测试模式不会上传身份信息或游戏记录。快速测试仍可直接体验现有游戏和本地报告。',

...
export class FormalOutbox {
  ...
  async flush(sessionId?: string, force = false) {
    if (this.flushing) return this.flushing
    this.flushing = this.flushOnce(sessionId, force).finally(() => { this.flushing = null })
    return this.flushing
  }
  private async flushOnce(sessionId?: string, force = false) {
    const now = Date.now()

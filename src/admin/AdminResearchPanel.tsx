...
import type { AdminFormalAssessmentReport, AdminResearchSession } from './adminTypes'
import { AdminFormalAssessmentReport as FormalReport } from './AdminFormalAssessmentReport'
...
export function AdminResearchPanel() {
  ...
  const [report, setReport] = useState<AdminFormalAssessmentReport | null>(null)
  ...
  const showReport = async (sessionId: string) => { setBusy(true); try { setReport(await adminResearchApi.getFormalReport(sessionId)) } finally { setBusy(false) } }
  ...
  return <section className="admin-research-panel" data-testid="admin-research-panel">
    ...
    {report && <section className="admin-research-detail" data-testid="admin-formal-report-dialog"><header><h3>评分报告</h3><button className="button button--ghost button--compact" onClick={() => setReport(null)}>关闭</button></header><FormalReport report={report} /></section>}
